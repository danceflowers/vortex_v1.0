...
#include "tensor_helper/tensor_core_test_utils.h"
#include <cstdio>
#include <cstdlib>
#include <ctime>
#include <cmath>
...
inline uint32_t reference_mul_fp22(uint16_t a_fp9, uint16_t b_fp9, RoundingMode rm) {
    const auto s1 = fmul_s1(a_fp9, b_fp9, 8, 14, rm);
    const auto s2 = fmul_s2(a_fp9, b_fp9, 8, 14, s1);
...
inline uint32_t reference_add4_fp22(uint32_t in0, uint32_t in1, uint32_t in2, uint32_t in3, RoundingMode rm) {
    return add4_fp22_rtl::add4_reference(in0, in1, in2, in3, rm);
}
...
uint32_t fp22_to_output_bits(uint32_t fp22, PrecisionType output_prec) {
    switch (output_prec) {
        case PREC_FP8_E4M3: return fp22_to_fp8_e4m3(fp22);
        case PREC_FP8_E5M2: return fp22_to_fp8_e5m2(fp22);
        case PREC_FP16:     return fp22_to_fp16(fp22);
...
void print_matrix_output_from_fp22(const char* title, const uint32_t m[8][8], PrecisionType out_prec) {
    printf("    %s\n", title);
    for (int i = 0; i < 8; i++) {
        printf("      ");
        for (int j = 0; j < 8; j++) {
            printf("%9.4f ", output_bits_to_double(fp22_to_output_bits(m[i][j], out_prec), out_prec));
        }
        printf("\n");
    }
}
...
void test_pipelined_throughput() {
    ...
    sim.reset();
     for (int job = 0; job < num_jobs; job++) {
        ...
        JobResult jr;
        tensor_core_test_utils::reference_matmul(ms.a_fp9, ms.b_fp9, ms.c_fp22, jr.ref, g_cfg.rm);
        reference_matmul_fp22_tree(ms.a_fp9, ms.b_fp9, ms.c_raw, out_prec, jr.ref_fp22, g_cfg.rm);
        results.push_back(jr);

        tensor_core_test_utils::load_inputs(&sim, ms.a_fp9, ms.b_fp9, ms.c_raw);
        if (tensor_core_test_utils::run(&sim)) {
     ...
     tensor_core_test_utils::load_invalid(&sim);
    ...
    for (int job = 0; job < num_jobs; job++) {
        ...
        print_matrix_output_from_fp22("OUTPUT", jr.fp22_out, out_prec);
    }
