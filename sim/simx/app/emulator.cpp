          ...
          if (tcu_type == TcuType::WMMA
           ...
           || tcu_type == TcuType::MMA_STORE) {
            ...
            if (core_->read_mma_descriptor(tcu_args.descriptor, &desc)) {
              ...
              tcu_args.transpose_a = 0;  // transpose 已移至 TMA descriptor
              tcu_args.transpose_b = 0;
            }
          }
  ...
  if (tcu_type == TcuType::MMA_LOAD || tcu_type == TcuType::MMA_STORE) {
    ...
    if (src0.type == RegType::Integer) {
      for (uint32_t t = 0; t < warp.tmask.size(); ++t) {
        ...
        os << " handle=" << tcu_args.runtime_handle;
        break;
      }
    }
  }

  auto score = tensor_unit_->scheduler_score(wid, tcu_type, tcu_args);
  auto block = tensor_unit_->classify_issue_block(wid, tcu_type, tcu_args);
  os << " score=" << score
